...
preprocess_features <- function(annotated_phos, residue, features_to_exclude = c("acc", "position", "mq_siteid", "PWM_nkinTop01", "PWM_nkinTop02", "sift_mean_score", "sift_acid_score", "sift_min_score", "quant_top10", "spectralcounts", "biological_samples")) {
    phos <- annotated_phos
    ## Exclude correlated and unnecessary features
    features_to_exclude <- c("acc", "position", "mq_siteid", "PWM_nkinTop01", "PWM_nkinTop02", "sift_mean_score", "sift_acid_score", "sift_min_score", "quant_top10", "spectralcounts", "biological_samples")
    ...
    phos <- phos %>%
        mutate(
            PEP = replace(PEP, PEP == 0, min(PEP[PEP != 0])),
            PEP = -log10(PEP),
    ...
    pre_proc_values <- preProcess(phos, method = c("center", "scale", "YeoJohnson", "nzv", "knnImpute"))
